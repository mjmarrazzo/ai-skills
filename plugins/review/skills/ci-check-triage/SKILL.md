...
Resolution order (identical to pr-review-triage so the two compose predictably):
...
If no PR open on the current branch:
...
> PR #`<num>` is `<state>`. Triaging checks on a closed PR is unusual — confirm you want to proceed.

Wait for confirmation.

### 2. Verify gh availability

Run `which gh && gh auth status` before any API call. On failure, surface the fix (`https://cli.github.com/` to install, `gh auth login` to authenticate) and stop. Never fall back to raw `curl` against the GitHub API — auth handling diverges and credentials leak.
...
```bash
# GitHub Actions checks: resolve the run, pull only the failing step logs
...
| **flaky / infra** | Timeout, network blip, runner OOM, a known-intermittent test, a cancelled run, "could not reach registry". The code is not implicated. | Offer to re-run the check. Never touch code. |
...
Apply the canonical active-workspace resolution (same as pr-review-triage and finish-branch): `WORKSPACE_PATH` param → enumerate `.claude-plans/*/` for `plan.md`/`spec.md` → one match use it, multiple prefer the branch's ticket key, zero → ad-hoc. When present, `plan.md` and `spec.md` help judge whether a failing test is testing in-scope behavior. In ad-hoc mode, classify from the log + diff alone and lower confidence one notch.
...
**Push:** `git push origin <branch>`. Plain push, never force, never rebase. The new commit re-triggers the checks. If the push is rejected (the remote moved): surface, do not force; tell the user to pull and re-run.
...
4. **Coverage / quality gates.** A failing coverage threshold isn't a bug to fix via debug-loop — it needs new tests, which is closer to scope-expansion. Currently classified `real failure` and handed over, but debug-loop may bounce it. Watch for this pattern.
