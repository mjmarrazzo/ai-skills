---
name: caveman
description: Use this skill when the user wants to SWITCH ON a persistent terse "caveman" output register — fragments, no filler, code/URLs/paths preserved verbatim — as a mode that stays on for the conversation, not a one-off formatting tweak. Triggers on "/caveman", "/caveman on", "/caveman off", "/caveman persist", "/caveman status", "talk like caveman", "caveman mode", "compressed output mode", "drop the filler from now on", "stay terse for the rest of this". The signal is register-as-a-mode: a toggle the user turns on (session-only or persistent, backed by `~/.claude/state/caveman.on` plus a SessionStart hook). Default state is OFF; the user opts in. Do NOT fire for a one-shot "make THIS answer shorter" request — "be terse", "shorter please", "shorter answers please", "less prose", "keep it brief", "just this once / on this one" are one-time asks Claude handles inline, NOT a register switch. The tell: scope a single reply → skip; scope the ongoing conversation → fire.
---
