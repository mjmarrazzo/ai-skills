...
Local knowledge is NEVER dropped. If local-knowledge alone exceeds 250 lines, the skill emits the local section and a single-line footer:

```
_[warning: local-knowledge section exceeds total budget; consider trimming .claude-knowledge/]_
```

It does NOT truncate local knowledge to fit. The user owns their gotchas log; the skill surfaces all of it.
