...
Replace guess-and-patch with disciplined root-cause analysis. When something fails — test, build, runtime exception, or wrong output — work through a reproducible, evidence-driven sequence that finds the actual cause before touching the fix. The loop terminates either with a confirmed fix and clean verify pass, or with an honest dossier of what was investigated and what remains unexplained.

Not in scope: performance profiling, security audits, architectural refactors triggered by accumulated technical debt. Those belong in blueprint. If debug-loop discovers that the failure is a symptom of a structural problem, it surfaces that finding and stops rather than spiraling into a rewrite.

## When to trigger

Pushy when something is broken, quiet otherwise.
...
Opt-out: "just revert it", "I'll debug this myself", "skip the analysis", or if the failure is obviously a missing environment variable the user already knows they need to set.
...
4. **The active workspace** — if a `.claude-plans/<active-dir>/` is present, read `handoff.md` for repo orientation and look for `decisions.md` to understand prior choices that might be relevant.
...
1. Run the failing test or command N=5 times, noting the failure rate and any pattern (always fails on iteration 2, only fails under load, fails if run after another test, etc.).
2. Attempt determinization: seed known RNG sources, mock the clock if time-dependent (`Date.now`, `time.time`), disable retries if the test framework has them, isolate the test from parallel execution. Run again.
...
**The constraint:** write down all hypotheses before executing any probe. Use this table:
...
Do not modify the production code during hypothesis testing. Temporary observations (prints, logs) are fine — track them (see Phase 6). Code changes before root cause is confirmed introduce new variables and contaminate the signal.
...
Before declaring verify complete, grep for `DEBUG-LOOP-TEMP` across all modified files. If any remain, remove them. Declaring done while any sentinel is present is a verification failure, not a minor omission.

If a `ui-validation` sibling is installed and the fix touched frontend code, invoke it now against the affected surfaces before declaring done.

## Phase 7: Log decision

If a `.claude-plans/<active-dir>/` workspace is active, append to `decisions.md`. If not, print the decision entry to chat.
...
- **Called by:** execute-plan (on task failure), ui-validation (on browser check failure), verify-before-done (on gate failure). Each caller is responsible for passing the failure bundle (error output + what was being attempted + what changed).
- **Calls:** ui-validation (at end of Phase 6 if frontend code was modified and ui-validation is installed); vscode-preview (to display the decisions.md entry if the sibling is installed — optional).
...
If a sibling is not installed: print a one-line notice ("if `ui-validation` were installed I'd run a browser check here") and continue without it.

## Open questions to resolve before SKILL.md

1. **Headed vs headless when invoked from debug-loop.** ui-validation defaults to headless; when debug-loop triggers it after a visual failure, headed mode is probably more useful. Should debug-loop pass a `headed: true` flag, or should that be the default when ui-validation is called from debug-loop? Leaning: debug-loop requests headed mode explicitly, since the user is actively debugging.
...
3. **N for intermittent failure characterization.** Five runs was chosen to balance signal vs. cost. For fast unit tests that's cheap; for slow integration tests it could be expensive. Should N be configurable by the caller, or should the skill choose N based on test duration? Leaning: caller can pass `max_flaky_runs`; default to 5.
