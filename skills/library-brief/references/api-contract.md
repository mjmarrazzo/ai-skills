...
The 5-intent cross-skill contract. Callers (`blueprint`, `pre-task-research`, user-direct) invoke library-brief with one of these intents. Long-lived contract; bumps require `schema_version` increment.
...
library: <canonical-name>            # required (all intents)
...
`caller=library-brief` is a no-op cycle guard. The skill logs a single-line error to stderr and returns without acting.
...
ecosystem: js
caller: user-direct
target_version: 7.0.0          # optional, default "latest" (resolved at write time)
sources: [homepage, readme, changelog, knowledge-capture]   # optional, default all available
...
- `library` must be a valid canonical name (lowercase, kebab-case per ecosystem convention in `brief-schema.md`).
- `ecosystem` must be a valid enum member.
- `target_version` of `"latest"` is resolved via WebFetch on the homepage/repo; the resolved semver is what gets persisted, NEVER the string `"latest"`.
...
```
~/.claude/data/library-briefs/js/react-router.md — wrote v7.0.0 (12 records across 4 sources)
```
...
| Body exceeds 200 lines after truncation pass | `"brief exceeded 200-line cap after truncation; assembled body surfaced for manual review (or open-questions.md in auto mode)"` |
...
2. `mv ~/.claude/data/library-briefs/<ecosystem>/<library>.md ~/.claude/data/library-briefs/<ecosystem>/<library>.v<N>.md`.
3. Audit log row:
   ```
...
ecosystem: js
caller: blueprint
target_version: 19.1.0          # optional, default upstream latest
...
- Brief must exist at `~/.claude/data/library-briefs/<ecosystem>/<library>.md`. If missing, refuse.
- `target_version` of `"latest"` (or omitted) resolves via WebFetch.
- Same-version refresh requires `fresh=true` to override the no-op guard.

**Delta detection:**

1. Read existing frontmatter → `prior_v = version_last_seen`.
2. Resolve `target_version`.
3. Fan out delta sources scoped to `prior_v → target_version`:
...
4. Compose ONE new Version history row.
...
```
~/.claude/data/library-briefs/js/react.md — refreshed 19.0.0 → 19.1.0 (1 version row, 3 body edits)
```

Or for no-op:

```
~/.claude/data/library-briefs/js/react.md — no changes (current at 19.1.0; pass fresh=true to force)
```
...
| Body exceeds 200 lines after truncation pass | Same refusal as `research_new` |
...
| `stale` | Brief found, `updated + stale_threshold_days < today` | Digest with `[stale]` prepended: `### library-brief: react (js, v19.1.0, [stale], updated 2024-11-02)` |
...
Rename a brief from `<library>.md` to `<library>.archived-<YYYY-MM-DD>.md` and drop from the README index. Used when a library is abandoned, replaced, or split into multiple packages.
...
1. `mv ~/.claude/data/library-briefs/<ecosystem>/<library>.md ~/.claude/data/library-briefs/<ecosystem>/<library>.archived-<YYYY-MM-DD>.md`.
2. Audit log row:
   ```
...
```
~/.claude/data/library-briefs/js/request.md → request.archived-2026-05-14.md (archived)
```
...
`caller=library-brief` on ANY intent → log a single-line error to stderr and return without acting. No skill in this monorepo invokes library-brief recursively; this guard exists for the case where a future skill chains through and accidentally lands back here.
...
```bash
test -f ~/.claude/skills/library-brief/SKILL.md || \
  ls ~/.claude/plugins/cache/**/skills/library-brief/SKILL.md 2>/dev/null
```
